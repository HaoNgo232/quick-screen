...
**What to build:** Refactor the native messaging host script to decouple OS-specific operations behind a clean `PlatformAdapter` seam, moving existing Linux behavior into a `LinuxAdapter` with 100% backward compatibility for `/tmp/quick-shot`, `xclip`, `wl-copy`, and `xdg-open`.
...
- [x] Implement `LinuxAdapter` preserving current xclip/wl-copy/xdg-open logic and `/tmp/quick-shot`.
